...
The list toolbar opens a full-screen map of the same feed. Tap a marker to open its details; Back returns to the map. Events without valid coordinates remain in the list and are omitted from the map with a notice. The initial camera is centered on the contiguous United States; worldwide markers remain available by panning or zooming. Returning from details preserves the camera position.

When the full-screen map first opens, it requests foreground location permission. If granted, Google Maps shows the standard current-location dot and recenter button. Approximate or precise location is sufficient. If permission is denied, location services are unavailable, or no location fix exists, all earthquake markers and map interactions remain available.
